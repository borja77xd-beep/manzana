...
public final class SikoEffectsCommand {
	...
	private static final List<String> SET_EFFECT_NAMES = Arrays.asList(
			"hojas", "sakura", "viento", "fenix", "rayos", "piedra", "vacio", "hielo", "buda", SikoEffectType.NONE_KEYWORD);
	private static final List<String> DEBUG_EFFECT_NAMES = Arrays.asList(
			"hojas", "sakura", "viento", "fenix", "rayos", "piedra", "vacio", "hielo", "buda");
	...
	private static int executeSet(CommandContext<ServerCommandSource> context, PlayerEffectStore store) {
		...
		if (type.isEmpty()) {
			context.getSource().sendError(
					Text.literal("Efecto desconocido: " + effectId + ". Usa hojas, sakura, viento, fenix, rayos, piedra, vacio, hielo, buda o ninguno."));
			return 0;
		}
	...
	private static int executeDebug(CommandContext<ServerCommandSource> context, AnimationManager animationManager) {
		...
		if (type.isEmpty()) {
			context.getSource().sendError(
					Text.literal("Efecto desconocido: " + effectId + ". Usa hojas, sakura, viento, fenix, rayos, piedra, vacio, hielo o buda."));
			return 0;
		}
