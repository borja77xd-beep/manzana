...
public class AnimationManager {
	...
	/**
	 * Duracion en ticks para un efecto concreto. FENIX y BUDA necesitan mas
	 * tiempo para desplegarse (fenix para la doble helice de fuego y el haz
	 * final; buda para trazarse, mantenerse formado y disolverse), asi que
	 * se les da el doble de la duracion configurada (con un minimo de 70
	 * ticks = 3.5s), independientemente del valor de duration_ticks usado
	 * por los demas efectos.
	 */
	private int resolveDuration(SikoEffectType type) {
		int base = Math.max(1, config.durationTicks);
		if (type == SikoEffectType.FENIX || type == SikoEffectType.BUDA) {
