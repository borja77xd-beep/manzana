...
| `rayos`   | Chispas electricas crepitando alrededor del cuerpo (particula del Sculk Shrieker), motas de luz flotando despacio, y dos rayos de verdad cayendo del cielo en zigzag durante la animacion, rematados con un flash brillante. Solo el efecto visual, sin dano ni sonido de trueno. |
| `piedra`  | La animacion "al reves": el jugador empieza envuelto en una cascara de piedra (textura vanilla real de la piedra) pegada al cuerpo, que se va desprendiendo a trozos con el tiempo -mas desprendimientos al principio, cada vez menos- hasta disiparse en una nube de polvo, dejandolo completamente libre. |
| `vacio`   | Un remolino de particulas de portal (la misma familia que el End y el teletransporte de los enderman) que converge desde fuera hacia el jugador, como si lo estuvieran materializando desde el vacio. Usa una unica particula de principio a fin, sin mezclar temáticas. |
| `hielo`   | Copos de nieve cayendo despacio y esparcidos alrededor del cuerpo, con destellos de escarcha ocasionales (textura vanilla real del hielo) formandose junto al jugador. Solo particulas de la misma familia fria. |
| `buda`    | Dibuja el manji budista (el simbolo tradicional del budismo/hinduismo/jainismo, usado incluso hoy en los mapas de Japon para senalar templos; en su orientacion vertical tradicional, sin el giro de 45 grados de la version asociada al nazismo) flotando en horizontal sobre la cabeza, trazandose poco a poco con luz calida (end_rod), manteniendose formado y disolviendose al final. Dura mas que los efectos cortos (~3.5s) para que de tiempo a leerse. |
...
```
/sikoeffects set <jugador> <hojas|sakura|viento|fenix|rayos|piedra|vacio|hielo|buda|ninguno>
```
...
```
/sikoeffects debug <hojas|sakura|viento|fenix|rayos|piedra|vacio|hielo|buda>
```
...
| `durationTicks` | Duracion total del efecto en ticks (20 ticks = 1 segundo). No aplica a `fenix` ni `buda`, que siempre duran al menos 70 ticks (~3.5s) para poder desplegar toda su secuencia. |
